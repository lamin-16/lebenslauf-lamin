        ...
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center gap-2">
            <div className="relative">
              <button onClick={() => setIsLangOpen(!isLangOpen)} className="flex items-center gap-2 px-3 py-2 border border-gray-300 rounded-lg bg-white text-sm font-medium text-gray-700 hover:border-royal-gold transition-colors">
              ...
              {isLangOpen && (
                <div className="absolute right-0 mt-2 w-40 bg-white border border-gray-200 rounded-lg shadow-lg py-1 z-50">
                  {languages.map(lang => (
                    <button key={lang.code} onClick={() => { setLanguage(lang.code); setIsLangOpen(false); }} className={`w-full text-left px-4 py-2 text-sm hover:bg-gray-50 transition-colors ${language === lang.code ? 'text-royal-gold font-semibold' : 'text-gray-700'}`}>
                      {lang.label}
                    </button>
                  ))}
                </div>
            ...
            <button onClick={onPrint} className="inline-flex items-center gap-2 btn-gold py-2 px-4 rounded-lg text-sm shadow-royal">
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden bg-white border-t border-gray-100 px-4 py-3">
          {navItems.map((item, idx) => {
            const Icon = item.icon;
            return (
              <a key={idx} href={item.href} onClick={(e) => handleNavClick(e, item)} className="flex items-center gap-3 py-3 text-sm font-medium text-gray-700 hover:text-royal-gold transition-colors border-b border-gray-50 last:border-b-0">
                <Icon className="h-5 w-5 text-royal-navy" />
                {item.label}
              </a>
            );
          })}
          <button onClick={onPrint} className="w-full flex items-center justify-center gap-2 btn-gold font-medium py-3 rounded-lg mt-3">
            <Printer className="h-4 w-4" />
